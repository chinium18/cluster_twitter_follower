...
def make_pairs(k):
    Sim_pair_array = [k]
    #Sim_pair_df = pd.DataFrame(np.zeros((1,len(key_list))), columns=key_list, index=k)
    v=dict_A[k]
    r=row_count[k]
    ...
    #    Sim_pair_df.loc[k,k2] = L_func(common_keys,r,r2,n) 

        Sim_pair_array.append(L_func(common_keys,r,r2,n))
    #return Sim_pair
    #return Sim_pair_df
    return Sim_pair_array
...
#        make_pairs(k, Sim_pair)
    ...
    Sim_pair_all_df = pd.DataFrame(Sim_pair_list, columns=['key']+key_list)
    Sim_pair_all_df.set_index('key', inplace=True)
    
    Sim_pair_all_df.to_parquet("./Sim_pair_mp/Sim_pair_"+str(index)+".parquet") 
